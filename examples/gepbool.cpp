...
// 4 terminal symbols (no arguments) a, b, 0 and 1
const int terminals = 4;
// terminal symbols first
const char symbols[nsymbols] = {'0','1','a','b','|','~','&'};
const int n = 2;   // for +,- and * which take 2 arguments
int h = 10;

int evalr(char *&e,int a,int b)
{
 switch(*(e++))
 {
  case '0': return 0;
  ...
  case 'a': return a;
  case 'b': return b;
  case '|': return evalr(e,a,b) | evalr(e,a,b);
  case '~': return (~evalr(e,a,b)) & 1;
  case '&': return evalr(e,a,b) & evalr(e,a,b);
  default : return 0;
 }
}

int eval(char *e,int a,int b)
{
 char *c = e;
 return evalr(c,a,b);
}
...
double fitness(char *c,int *data,int N)
{
 int d, sum = 0;
 
 for(int j=0;j<N;j++)
 {
  d=eval(c,data[3*j],data[3*j+1])^data[3*j+2];
  sum += d;
 }
 return sum;
}
...
void gep(int *data,int N,int P,double eps)
{
...
int main(void)
{
 ...
 int data[] = {0,0,1, 0,1,1, 1,0,0, 1,1,1};
 gep(data,4,30,0.5);
 cout << endl;
 return 0;
}
