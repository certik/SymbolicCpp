...
int random(int n)
{
 return floor(double(rand())/RAND_MAX * n);
}
...
void gep(const Symbolic &integrand,int P,double eps)
{
 ...
 for(i=0;i<pop_len;i++)
  if(i%gene_len < h) population[i] = symbols[random(nsymbols)];
  else population[i] = symbols[random(terminals)];
 ...
 while(bestf >= eps)
 {
  ...
  for(i=0;i<pop_len;)
  {
   ...
   if(double(rand())/RAND_MAX < precomb)
   {
    ...
    replace = i-gene_len;
    rlen = random(gene_len);
    // a random gene for recombination
    replace2 = (random(P))*gene_len;
